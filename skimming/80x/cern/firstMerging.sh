...
export MERGINGDIR=/eos/cms/store/caf/user/ceballos/Nero/merging_80x;
if [ $# == 1 ] && [ $1 == 1 ]; then
...
do
    ...
    find $SKIMDIR/* -name ${theFolder}| awk '{split($1,a,ENVIRON["SKIMDIR"]);split(a[2],b,"/");printf("hadd -O -f %s/%s_%s.root %s/*.root\n",ENVIRON["MERGINGDIR"],b[3],ENVIRON["theFolder"],$1);}' >> $theRND.sh;
done

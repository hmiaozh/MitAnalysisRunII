...
unsigned int period = 0
  ...
  for(UInt_t ifile=0; ifile<infileName_.size(); ifile++) {
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      if(TMath::Abs(thePandaFlat.looseLep1Eta) >= 2.4 || TMath::Abs(thePandaFlat.looseLep2Eta) >= 2.4 || TMath::Abs(thePandaFlat.looseLep3Eta) >= 2.4) continue;

      if(thePandaFlat.looseLep1Pt <= 25 || thePandaFlat.looseLep2Pt <= 20 || thePandaFlat.looseLep3Pt <= 10) continue;
      ...
      bool passLooseLepId = ((thePandaFlat.looseLep1SelBit & kFake) == kFake) && ((thePandaFlat.looseLep2SelBit & kFake) == kFake) && ((thePandaFlat.looseLep3SelBit & kFake) == kFake);
      ...
      else if(abs(thePandaFlat.looseLep1PdgId)==11 && abs(thePandaFlat.looseLep2PdgId)==11 && abs(thePandaFlat.looseLep3PdgId)==11) lepType = 3;
      else if(abs(thePandaFlat.looseLep1PdgId)==11 && abs(thePandaFlat.looseLep2PdgId)==11) lepType = 1;
      else if(abs(thePandaFlat.looseLep1PdgId)==11 && abs(thePandaFlat.looseLep3PdgId)==11) lepType = 1;
      else if(abs(thePandaFlat.looseLep2PdgId)==11 && abs(thePandaFlat.looseLep3PdgId)==11) lepType = 1;
      else if(abs(thePandaFlat.looseLep1PdgId)==13 && abs(thePandaFlat.looseLep2PdgId)==13) lepType = 2;
      else if(abs(thePandaFlat.looseLep1PdgId)==13 && abs(thePandaFlat.looseLep3PdgId)==13) lepType = 2;
      else if(abs(thePandaFlat.looseLep2PdgId)==13 && abs(thePandaFlat.looseLep3PdgId)==13) lepType = 2;
      else {printf("Impossible dilepton combination: %d %d %d\n",thePandaFlat.looseLep1PdgId,thePandaFlat.looseLep2PdgId,thePandaFlat.looseLep3PdgId); continue;}
	  ...
	  vZl2 = vLoose2;
	  vWln = vLoose3;
	  tightWlnId = (thePandaFlat.looseLep3SelBit & kTight) == kTight && (thePandaFlat.looseLep3SelBit & kDxyz) == kDxyz;
	  whichWln = 2;
	}
      }
	  ...
	  vZl2 = vLoose3;
	  vWln = vLoose2;
	  tightWlnId = (thePandaFlat.looseLep2SelBit & kTight) == kTight && (thePandaFlat.looseLep2SelBit & kDxyz) == kDxyz;
	  whichWln = 1;
	}
      }
	  ...
	  vZl2 = vLoose3;
	  vWln = vLoose1;
	  tightWlnId = (thePandaFlat.looseLep1SelBit & kTight) == kTight && (thePandaFlat.looseLep1SelBit & kDxyz) == kDxyz;
	  whichWln = 0;
	}
      }
      ...
      bool passSel[6] = {mllmin > 4, fabs(mllZ-91.1876) < 15, vWln.Pt() > 20, vMet.Pt() > 30, (vLoose1+vLoose2+vLoose3).M() > 100, thePandaFlat.jetNMBtags == 0};
	...
	if(vLoose3.Pt() > 0) if(vLoose3.DeltaR(vJet[nj]) < 0.3) isLepton = kTRUE;
      ...
      if(theCategory != 0){
        double the_eta_sf[3] = {1.0, 1.0, 1.0};
        ...
        if(abs(thePandaFlat.looseLep3PdgId)==13){
          ...
          double etal = thePandaFlat.looseLep3SCEta; if(etal >= 2.4) etal = 2.3999; else if(etal <= -2.4) etal = -2.3999;
          int binEta = scalefactors_Electron_Eta->GetXaxis()->FindFixBin(etal);
          the_eta_sf[2] = scalefactors_Electron_Eta->GetBinContent(binEta);
        }

        double sfWeightLepEff[3] = {1.0, 1.0, 1.0};
        if(abs(thePandaFlat.looseLep1PdgId)==13){
          double etal = thePandaFlat.looseLep1Eta; if(etal >= 2.4) etal = 2.3999; else if(etal <= -2.4) etal = -2.3999;
      ...
      if(passWZSel)          {histo[lepType+45][theCategory]->Fill(TMath::Min((double)idJet30.size(),4.499),totalWeight);         histo[49][theCategory]->Fill(TMath::Min((double)idJet30.size(),4.499),totalWeight);}
      if(passTopSel)         {histo[lepType+50][theCategory]->Fill(TMath::Min((double)idJet30.size(),4.499),totalWeight);         histo[54][theCategory]->Fill(TMath::Min((double)idJet30.size(),4.499),totalWeight);}

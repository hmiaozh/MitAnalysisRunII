...
unsigned int period = 0
  ...
  for(UInt_t ifile=0; ifile<infileName_.size(); ifile++) {
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      bool passFakeTrigger = ((thePandaFlat.trigger & kMuFakeTrig) == kMuFakeTrig || (thePandaFlat.trigger & kEGFakeTrig) == kEGFakeTrig);
      bool passPresel = false;
      if(passFakeTrigger == true){
        double mll = 0.0;
	if(thePandaFlat.nLooseLep == 2){
          TLorentzVector lep1;
          lep1.SetPtEtaPhiM(thePandaFlat.looseLep1Pt,thePandaFlat.looseLep1Eta,thePandaFlat.looseLep1Phi,0.0);
          TLorentzVector lep2;
          lep2.SetPtEtaPhiM(thePandaFlat.looseLep2Pt,thePandaFlat.looseLep2Eta,thePandaFlat.looseLep2Phi,0.0);
	  mll = (lep1 + lep2).M();
        }
      ...
      if(thePandaFlat.nLooseLep == 2) {
        ...
        if     (abs(thePandaFlat.looseLep1PdgId)==13) lepType = 0;
        else if(abs(thePandaFlat.looseLep1PdgId)==11) lepType = 1;
	...
	if(vLoose2.Pt() > 0) if(vLoose2.DeltaR(vJet[nj]) < 0.3) isLepton = kTRUE;
      ...
      bool passZSel = thePandaFlat.nLooseLep == 2 && passMllSel && (lepType == 0 || lepType == 1) && vLoose1.Pt() > minLepPt[lepType] && vLoose2.Pt() > minLepPt[lepType] && 
                     (thePandaFlat.looseLep1SelBit & kFake) == kFake && (thePandaFlat.looseLep2SelBit & kFake) == kFake;
      bool passWSel = thePandaFlat.nLooseLep == 1 && mt > 50.0 && vMet.Pt() > 30.0 && vLoose1.Pt() > minLepPt[lepType] && 
                     ((thePandaFlat.looseLep1SelBit & kFake) == kFake);
	...
	histoWlnPt[lepType][theCategory]->Fill(TMath::Min(vLoose1.Pt(),44.999),totalWeight);
      ...
      if(passFakeNumSel == true) {
        if((thePandaFlat.looseLep1SelBit & kFake)   == kFake  ) histoFakeDenEtaPt[lepType][theCategory]    ->Fill(TMath::Min(TMath::Abs(vLoose1.Eta()),2.499),TMath::Min(vLoose1.Pt(),44.999),totalWeight);
        if((thePandaFlat.looseLep1SelBit & kFake)   == kFake &&
	   (thePandaFlat.looseLep1SelBit & kMedium) == kMedium) histoFakeNumSel0EtaPt[lepType][theCategory]->Fill(TMath::Min(TMath::Abs(vLoose1.Eta()),2.499),TMath::Min(vLoose1.Pt(),44.999),totalWeight);
        if((thePandaFlat.looseLep1SelBit & kFake)   == kFake &&
	   (thePandaFlat.looseLep1SelBit & kTight) == kTight)   histoFakeNumSel1EtaPt[lepType][theCategory]->Fill(TMath::Min(TMath::Abs(vLoose1.Eta()),2.499),TMath::Min(vLoose1.Pt(),44.999),totalWeight);
        if((thePandaFlat.looseLep1SelBit & kFake)   == kFake &&
	   (thePandaFlat.looseLep1SelBit & kTight) == kTight &&
	   (thePandaFlat.looseLep1SelBit & kDxyz) == kDxyz)     histoFakeNumSel2EtaPt[lepType][theCategory]->Fill(TMath::Min(TMath::Abs(vLoose1.Eta()),2.499),TMath::Min(vLoose1.Pt(),44.999),totalWeight);
      }
